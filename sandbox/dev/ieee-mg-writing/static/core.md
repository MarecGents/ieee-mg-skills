...
| 维度 | 定量特征 | 约束 |
|------|----------|------|
| **被动语态比例** | 分章节估算值：Abstract 40%-50%、Introduction 35%-45%、Results 30%-40%、Motivation 30%-40%、Conclusion 40%-50%（人工估算，无 POS 统计佐证；权威口径见 quantitative-baseline.md §2） | 💡 |
| **第一人称频率** | "we" 平均每千词约 6-8 次（Abstract 实测 6.5/千词，16/21 篇使用 ≈76%），用于 propose/derive/formulate 等动作。分章节口径：Introduction 5-10、System Model 10-18、Results 3-7、Conclusion 5-10（见 quantitative-baseline §3） | 💡 |
...
| 维度 | 定量特征 | 约束 |
|------|----------|------|
| **平均句长** | 定性：Abstract 偏短、Introduction 偏长（抽样口径：Abstract 约 25-29 词/句）；句子控制在 15-30 词为宜（**通用参考，各章节区间见 quantitative-baseline §4**） | 💡 |
...
| 全双工/半双工 | Full-Duplex (FD) / Half-Duplex (HD) |
| 解码转发中继 | Decode-and-Forward (DF) Relay |
| 性能指标/场景缩写 | 完整清单见 ../ieee-mg-share/static/terminology.md §2.4-2.6 |
...
- **素材缺失标注约定**：结果数据/仿真参数/图表缺失时在稿中以 `[待补: 描述]` 标注，不虚构数值；首创性/空白声明（"this paper is the first"、"there is no existing work"）须用户确认领域无此工作，否则用弱声明并标注 `[待核]`，禁止虚构空白。
