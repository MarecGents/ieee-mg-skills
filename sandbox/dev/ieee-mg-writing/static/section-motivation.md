...
**开头模式选型（v1.3.0）**：综述回顾后接转折→While/Although；承接前文章节→The aforementioned/Building upon；直接进入本文视角→直接陈述。
...
**贡献引导句变体（v1.3.0，expression-bank §4）**："The main contributions of this paper can be summarized as follows"（最高频）/ "The primary contributions ... are summarized as follows" / "The major contributions ..." / "The essential contributions ..." / "The basic contributions of the thesis ..." / "In summary, the following are the primary contributions..."（约 15/21 篇使用引导句）。
...
**使用决策（v1.3.0）**：当贡献跨 ≥3 个特征维度且与 ≥3 篇已有工作存在正交差异时推荐使用对比表；否则可跳过（语料仅约 5% 论文使用）。
...
- [✅] 是否有至少 3 个不同的贡献点
- [✅] 贡献点动作动词多样性是否 ≥3 类（propose/derive/analyze/demonstrate 中 ≥3 类）
- [💡] 本文贡献与已有工作的差异是否清晰
- [💡] 贡献点之间是否有逻辑递进关系
...
### 4.2 研究空白句变体（语料实测全收录）
- "X is far from being well understood."（语料 3/21 篇）
- "This paper presents the first comprehensive analysis of X."（MF-RIS 篇首创性变体）
