...
**Layer 1 打开方式选型（v1.3.0）**：领域整体发展趋势→"With the development/advance of X"；技术代际推进叙事→"As X advances"；承接上文/多篇连写→直接陈述；RIS 类技术定位→"X has been deemed as"。
...
**Layer 2 演进模式选型（v1.3.0）**：单一技术演进→模式 A（问题驱动）；多方案横向对比→模式 B（对比驱动）；代际更替→模式 C（时间线驱动）。
...
**方式 A：编号段落式（1) [Category]: ...）**（约 10%，2/21 篇，如 STAR-RIS 篇 "1) Related Works on RIS:" / "2) Related Works on RIS-NOMA:"）
```
...
**方式 B：子节式（### A. Previous Works + 编号段落）**（SGF 篇实证）
```
### A. Previous Works
...
**综述组织选型（v1.3.0）**：综述跨 ≥2 个平行主题→编号段落式（方式 A）；主题有层级或需子节展开→子节式（方式 B）；单主题→不分块。
...
| **对比引用** | "Different from [x], [y], we..." / "In contrast to [x] which considered..., this paper..." | 语料实测高频变体 |
...
**空白句式变体（语料实测，均可用）：**（⚠ 边界：空白/首创性声明须用户确认领域无此工作，否则用弱声明并标注 [待核]，禁止虚构空白）
