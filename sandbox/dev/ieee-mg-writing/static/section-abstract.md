...
**开头模式选型（v1.3.0）**：全新系统/新框架→模式 1（This paper proposes/introduces）；成熟技术新场景→模式 2（技术主语式）；纯理论分析→模式 1（investigates）或模式 3（被动式）；存在明确技术瓶颈/双面性→模式 4（Although 让步式）。
...
> **写/不写决策（v1.3.0）**：领域成熟、本文为增量改进→写问题句；全新方向/本文即首开→省略，直接进入方法句。
...
**编号风格选型（v1.3.0）**：i) 系与 1) 系均为语料实证（i) 系 9 组 / 1) 系 14 组）；编号风格与正文章节列表编号统一，纯并列发现用 1)，含层级/子项时用 i)。
...
- [✅] 背景句是否点明了研究领域的重要性（判据：首句含领域名词 + 重要性词 growing interest/promising/indispensable/advance 之一）
...
- [✅] 是否包含至少一个性能比较（判据：含 outperform/exceed/be superior to/precede that of/superior to 之一即可）
- [✅] 词数是否在 85-280 词范围内（典型 130-260）
- [✅] 是否使用 IEEE 数学符号规范（判据：摘要通常无公式，若含数学符号则须符合斜体/粗体规范；无符号视为通过）
- [✅] 时态是否统一（现在时 + 现在完成时）
- [💡] 是否在一条结果中指出了特殊发现或权衡关系（语料高频特色）
- [❌] 是否未引入正文未讨论的新内容
- **[待补] 约定（v1.3.0）**：结果数据/数值缺失时以 `[待补: 结果 i 的具体数值]` 标注，不虚构数值；用户未提供结果内容时先询问，不自行编造。
