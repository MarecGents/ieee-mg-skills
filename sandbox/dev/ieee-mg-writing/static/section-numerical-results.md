...
**子场景组织选型（v1.3.0）**：多图且每图独立问题→按图（方式 A）；单指标多参数扫描→按指标（方式 B）；单参数系统扫描→按参数（方式 C）。
...
**Step 3：解释原因（1-2 句）（v1.3.0 补变体）**——"The reason behind this phenomenon is that" / "The basic reason for this phenomenon is that"（各 2 行）、多原因 "The reasons are that: 1)... 2)... 3)..."、归因 "This comes from the fact that"。**❌ 注意**："This is because that" 为语料高频错误，勿用
```
...
- [💡] 结果是否与纯理论分析章节互相印证（判据：引用 Remark/Theorem/Corollary 编号，或明确"与理论曲线对比"）
- [✅] 是否避免了单纯的图表数据罗列（判据：每图分析段含 ≥1 句观察 + ≥1 句解释（This is because / The reason is that）即通过）
...
- 仿真参数/图/结果数据缺失时以 `[待补: 描述]` 标注或询问用户，**不得编造仿真结果**；
- 参数表数值仅为示意（语料实测 Rician 因子多为 -5/-7 dB、MC 主流 10^6），以用户实际仿真为准；
- MC 次数应落在语料分布（10^6 主流 / 10^5 次主流 / FL 10 iterations）或用户实测值。