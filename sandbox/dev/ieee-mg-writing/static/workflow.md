...
- 明确篇幅要求（conf 场景按 section-architecture 判断边界适配：Abstract 100-180 词、贡献 2-3 点、综述压 1 段）
- **信息缺失处置**：用户未提供技术主题/场景/目标期刊时，先询问用户（见 SKILL.md 判断边界）；不虚构内容

### 第二步：结构规划
根据章节模板规划内容框架，确定：
- 主要论点和技术贡献（3-5 个贡献点；**数量决策**：贡献点数=问题数=空白数，单系统场景 3 点、多场景/多指标 4-5 点；推荐 提出/推导/优化/验证 四动作，顺序可灵活）
- 所需的数学推导和优化问题
...
### Abstract 特征
- 约 43%（9/21）以 "This paper/This article investigates/proposes/introduces..." 开头；其余为技术主语式（"X has attracted growing interest..."）、被动式（"A X system is investigated"）、"Although..." 让步式（约 5%）等（四模式口径与 section-abstract 一致）
- 约 76%（16/21）的摘要使用 "we"
