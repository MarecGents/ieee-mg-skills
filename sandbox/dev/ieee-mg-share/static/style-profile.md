...
| 维度 | 定量特征 | 约束 |
|------|----------|------|
| **被动语态比例** | 分章节估算值：Abstract 40%-50%、Introduction 35%-45%、Numerical Results 30%-40%、Motivation 30%-40%、Conclusion 40%-50%（人工估算，无 POS 统计佐证；权威口径见 quantitative-baseline.md §2） | 💡 |
| **第一人称频率** | "we" 平均每千词约 6-8 次（Abstract 实测 6.5/千词），16/21 篇摘要使用（≈76%），用于 propose/derive/formulate/demonstrate 等动作 | 💡 |
| **主要情态动词** | 排序：can > may > could > might（定性印象，无计数依据）。选型：can=能力/可能性（最常用）；may=许可或允许的可能性；could=委婉假设/条件可能；might=低概率推测。Introduction 内 may/could/might 用例极少，慎用 | 💡 |
...
| 维度 | 定量特征 | 约束 |
|------|----------|------|
| **平均句长** | 定性：Abstract 偏短、Introduction 偏长。抽样口径：6 篇 Abstract 抽样均值约 25-29 词/句（含编号结果长句）。**判定方法**：以句号/问号/感叹号为句界切分（分号不算；引用编号列表整体计 1 句；公式/图表标题不计句），计算 总词数÷句数 | 💡（参考区间，不作判错依据） |
...
| **Introduction 引用句三型** | 三型并存：① 一般过去时 "The authors in [x] proposed/investigated..."；② 现在完成时 "Prior studies have demonstrated..." / "have proposed"；③ 一般现在时 "the authors of [14] demonstrate/model..." | — | 选型：持续影响/通用结论→现在完成时；具体历史动作/带时间点→过去时；当前范例/作者观点→现在时。语料三型均高频 | 💡 |
...
| **Conclusion** | 现在完成时（回顾工作，约 15/21 篇） | 一般现在时（总结发现）；一般过去时/现在时为合法变体（约 6/21 篇） | 选型：句含完成时标记词（has/have）→用现在完成时；无标记词且回顾已发生动作→可自然转过去时；陈述仍成立的发现→现在时。"In this paper, we investigated..."（2 篇）等变体语料实证合法，不强制 | 💡 |
...
> 按功能分类，按使用频率降序。**频次统一采用 logic-connectors.md 的全语料 6 文件口径（权威）**，本表为摘要，详细星级/计数见 logic-connectors.md §0-§5；连接词频次冲突时以 logic-connectors.md 为准。

| 功能 | 首选词 | 次选词 | 使用场景示例 | 约束 |
|------|--------|--------|-------------|------|
| **因果** | due to（≈222 行） | because of（≈6 行，少用）, owing to（≈9 行）, as a result（≈23 行）, thereby（≈27 行） | 结果解释、原因分析 | 💡 |
| **转折** | however（≈105 行） | although（≈20 行）, whereas（≈13 行）, in contrast（≈18 行）, nevertheless（≈8 行）, while | 研究空白、结果对比 | 💡 |
| **递进** | furthermore（≈169 行） | in addition（≈156 行）> additionally（≈123 行）≈ moreover（≈121 行，同档并列）> besides（≈8 行）, notably（≈4 行，语料极少） | 贡献列点、补充说明 | 💡 |
| **对比** | compared to / compared with（两者均高频，均为规范英语） | in contrast, on the other hand, conversely, alternatively | 方案比较、基准对比 | 💡 |
| **举例** | such as（≈29 行） | e.g.（≈8 行）, for example（≈6 行）, for instance, including | 技术列举 | 💡 |
| **强调** | specifically（≈102 行） | in particular（≈30 行）> particularly, especially, notably（≈4 行，语料极少） | 重点发现 | 💡 |
| **结论** | hence（≈83 行）/ thus（≈52 行）/ therefore（≈42 行） | consequently（≈16 行）, accordingly, as a result（≈23 行） | 总结推导 | 💡 |
| **条件** | provided that | subject to, given that, under the condition that | 约束条件 | 💡 |

> 注：连接词选择以功能与上下文为准，不必刻意堆砌高频词；同段内同功能词不超过 2 次（详见 logic-connectors.md）。频次为全量口径估算，详细星级见 logic-connectors.md §0。
...
- 连接词频次冲突时以 **logic-connectors.md** 为准；词数/密度等定量口径以 **quantitative-baseline.md** 为准；本文件引用他处数据不一致时，以被引用文件为准。
