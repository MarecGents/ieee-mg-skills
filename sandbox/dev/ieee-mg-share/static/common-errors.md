...
| "This is due to that" | "This is due to **the fact that**" | ✅ | 语料 5+ 处（Numerical Result.md:865, 946, 1019, 1069, 1359 等） |
| "Regarding to" | "Regarding" 或 "With regard to" | ✅ | Introduction.md:344, 476 等 |
| "by the virtue of" | "by virtue of" | ✅ | Conclusion.md:108 等 4+ 处 |
...
| 同一段落内时态反复切换 | 按功能分句保持时态一致。**判定**：同一功能块（如"描述图内容"或"解释原因"）内不得混时态；换功能块需有标记词（如 This is because / In addition）提示 | ✅ | 通用规则 |
...
## 8b. 拼写/音近字错误（v1.3.0 语料实测）

| 错误 | 正确 | 分级 | 错误来源 |
|------|------|:----:|----------|
| "my bring about / my give rise to" | "**may** bring about / may give rise to" | ✅ | Conclusion.md:60, 84, 102（3 篇高频笔误） |
| "treaties"（误） | "**trials**" | ✅ | Conclusion.md:102 |
| "high-date" | "**high-data**" | ✅ | Conclusion.md:60 |
| "certificate"（误） | "**certifies**" | ✅ | Numerical Result.md:979 |
| "the superior of X"（名词化） | "the **superiority** of X" | ✅ | Conclusion.md:114；Numerical Result.md:1162, 1194 |

> 拼写/音近字错误为 light 深度必改项；语料实证高频（may 笔误 3 篇），polishing 需重点排查。
...
- **本表仅 polishing/reviewer 使用**：writing 起草时不主动引入错误句式，直接使用正确形式；
- **未收录表达的处理**：本表未覆盖的表达按通用语法判断，不强行套用表内规则；
- **语料合法变体保护**：语料实证的合法表达（如 compared with、better、Fig. X plots 现在时）不得判为错误（见 §2 说明）；
- **判定优先级**：本表为错误判定权威；与 polishing core.md 同源条目冲突时以本表实测为准。