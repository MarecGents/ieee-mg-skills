...
> **判断边界（v1.3.0）**：① 句式选择以语义契合为准，不刻意堆砌高频句式；② 语料未收录的学术表达若符合 IEEE 惯例可用（课题组规范），不视为违规；③ 句式频次冲突时以本文件为准（本文件为句式库权威），与其他文件数值口径不同系统计范围差异。
...
| **背景受关注** | "X has attracted growing research interest / received a lot of attention due to its ability to [优势]." | ✅ 语料实测（多篇摘要/引言开头高频） |
...
| "X is far from being well understood." | 领域理解不足 | 💡 语料实测（3/21 篇） |
...
| **进一步推进** | "As a further advance/development, ..." | 💡 语料实测（全语料 15+ 处，Abstract/Introduction 高频） |
| **动机引出** | "Motivated by this gap/inspiration, this paper seeks to investigate..." | ✅ 语料实测（Motivated by this/these 全语料 15+ 处） |
| **暗示结论** | "This suggests that ..." | 💡 语料实测（NR 节高频，用于结果到结论衔接） |
