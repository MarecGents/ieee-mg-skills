...
### 子场景常见划分方式（选型决策 v1.3.0）
- 方式 A：按图组织（Fig. X→分析，Fig. Y→分析）——**适用**：多图且每图独立问题
- 方式 B：按性能指标（Outage / Ergodic Rate / Throughput）——**适用**：单指标多参数扫描（语料多篇实证）
- 方式 C：按参数影响（SNR / Elements / Power）——**适用**：单参数系统扫描
...
- **结构模板为软约束基准**：与目标期刊章节规范冲突时以目标期刊为准（如 conf 允许合并 Motivation 与 Introduction）；
- **单章写作/审核**：只取对应章节的结构定义，不强制输出其他章节；
- **数值口径**：本文件占比/词数为语料实测估算，与 quantitative-baseline.md 冲突时以后者为准；
- **conf/mag 适配**（课题组建议，非语料实测——语料均为 IEEE Trans journal）：conf 摘要 100-180 词、编号结果 2-3 条、结论 60-120 词、文献综述压至 1 段、贡献 2-3 点；mag 摘要 100-200 词、可含教程式/综述式说明段。