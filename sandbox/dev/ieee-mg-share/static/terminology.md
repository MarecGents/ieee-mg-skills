...
> **约束等级（v1.3.0）**：✅ = 硬约束（标准写法必须遵守）；💡 = 软约束（课题组建议缩写/写法，可按目标期刊调整）。术语表各行的标准写法均为 ✅（硬约束），标注"课题组建议缩写"的缩略语为 💡。
...
| 无线携能通信 | Simultaneous Wireless Information and Power Transfer (SWIPT)（语料两种写法并存：simultaneous wireless power transfer / simultaneous wireless information and power transfer，主用信息+功率版） |
...
| 每信道使用比特 | Bit Per Channel Use (BPCU)（语料 NR 节多处定义，如 "0.01 BPCU"） |
| 人工噪声 | Artificial Noise (AN)（语料物理层安全多篇使用） |
...
- **Abstract 内首次出现** → 定义（语料实测 ipSIC 12/21、pSIC 11/21 篇摘要内即定义）；
- **正文首次出现** → 无论 Abstract 是否已定义，正文首次出现处仍需定义（Abstract 独立于正文）；
- **标题** → 可含缩略语（语料 18/21 篇标题含），但正文首次出现处仍须全称定义；
- **全文只出现一次的术语** → 可只写全称不定义缩写（避免无意义缩写）。
