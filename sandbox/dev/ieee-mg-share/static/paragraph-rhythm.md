...
每个段落的第一句应为 **topic sentence**，概括本段内容。课题组语料中 90%+ 的段落遵循此规则。**判定方法**：删除首句后，若剩余段意无法预测（不知道本段讲什么），则首句为 topic sentence；若删除后段意完整，则首句非 TS。
...
**✅ 必须（违规即修改，moderate 深度起——过渡手法/节奏属句式层改动，超出 light 边界；light 仅语法/拼写/标点）**：
- [ ] 相邻两句未使用同一种过渡手法？
...
## 5. 段落长度分布（v1.3.0 与 quantitative-baseline 统一；注意本表混列「章节总词数」与「段级长度」，用途不同）

| 章节 | 推荐段长（词数） | 原因 |
|------|:----------------:|------|
| Abstract（章节总词数） | 全部 85–280 词 | 单一段落（语料实测 130–260，与 quantitative-baseline.md 统一） |
...
| Conclusion（章节总词数） | 全部 80–200 词 | 单一段落（21/21 篇；实测 60–200，均值约 145，0 篇超 200；80 为去极值建议下限） |

## 6. 节奏变奏技巧

**触发选择**：长句密集段（连续 >25 词句 ≥3 句）→ 用 6.1 短句突击；存在 3 个平行发现/优势 → 用 6.2 并列三连；写动机/贡献过渡 → 可用 6.3 问答节奏（低频，勿滥用）。
...
- **节奏基准为软约束（💡）**：与目标期刊风格或审稿人偏好冲突时，以目标期刊为准；
- **判定方法统一**：数句/数过渡词/数手法均按 §4.5 的方法（句号切分、过渡词集合、七种手法分类）；
- **节奏规则不适用于技术密集段落**：公式推导段、参数表说明段可豁免过渡多样化要求。