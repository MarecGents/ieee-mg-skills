...
| 配对 | 模式 |
|------|------|
| ~~"Although ... however"~~ | 已删除（语料 0 命中，无源示例） |
...
- **以功能与上下文为准**：连接词选择优先匹配逻辑关系（因果/转折/递进/对比），不刻意堆砌高频词；
- **同段同功能词 ≤2 次**：同一功能（如递进）在同一段落内最多使用 2 个不同连接词，避免单调；
- **同一连接词不连续两句开头**：句首连续使用同一连接词视为节奏缺陷（判定法见 paragraph-rhythm §4.5）；
- **频次裁决**：本文件为连接词频次唯一权威；style-profile §5 引用本文件数值，冲突时以本文件为准；
- **语料未收录的连接词**（如 Henceforth、Ergo 等）慎用，除非目标期刊惯例支持。