...
- [ ] 2. 仿真参数表（TABLE I 或 TABLE II）是否存在（判定：引言无对比表→参数表为 TABLE I；引言已有对比表→参数表续编 TABLE II，依据 baseline §7：TABLE I 11 篇 + TABLE II 8 篇）
...
- [ ] 12a. MC 次数是否落入语料主流口径（baseline §7：10^6 7/12、10^5 4/12、FL 10 iterations；🟢 提示级）

## 常见致命问题（14 项全映射）
| 问题 | 触发检查项 | 严重性 |
|------|-----------|:------:|
| 无仿真参数表 | #2 | 🟡（语料 2/21 篇无参数表为合法变体，无表结合上下文判定，不直接判 🔴） |
| 图分析<4句 | #5 | 🟡 |
| 无原因解释 | #6 | 🟡 |
| 无理论-仿真对照 | #7 | 🟡 |
| 无任何基准方案 | #3 | 🔴 |
...
| 子场景无组织 | #12 | 🟢 |
| 无多因解释 | #11（子项 a） | 🟢（低频手法之一，非独立必查） |
| 无 trade-off 讨论 | #11（子项 b） | 🟢（低频手法之一，非独立必查） |
