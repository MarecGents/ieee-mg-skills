...
> **核心结构**：工作回顾 → 主要发现 →（可选）数值总结 →（可选）未来工作
> **约束分级（v1.2.0）**：✅ 硬约束 / 💡 软约束。
...
**开头句式（语料实测分布，21 篇）：** 注意：语料中不存在 "In this paper, we have investigated" 这一**精确**开头（但近邻形式 "In this paper, we investigated" 2 篇、"In this article, we have studied" 1 篇存在，均合法）。实际开头分布如下：
...
**开头模式选型（v1.3.0）**：以系统/研究对象为主语的客观回顾→模式 1（被动完成时）；强调论文整体贡献→模式 2（This paper has）；第一人称叙事/给出方法+对象→模式 3（In this paper, we investigated）；非 Trans 期刊（Magazine）→模式 4（This article）。

**模式 4（约 14%）：This article 变体 / 被动现在时**
```
...
- "In this paper, the physical layer security issues are discussed exhaustively..."（被动现在时变体）

**时态说明**：现在完成时在句中回顾具体动作时使用（"we have derived/investigated" 等，语料 9 处）；开头句本身以被动完成时（模式 1）与 "This paper has + 过去分词"（模式 2）为主，但模式 3/4 为语料实证合法变体，**不强制现在完成时**。
...
> 注：**具体数值为可选项**（语料仅 1/21 篇结论含具体数值——FL 篇 72%/58%/50%）；多数结论定性复述关键发现即可，数值详见于正文。

### 要素 4：未来工作（💡 1-2 句，实测约 48%，10/21 篇包含）
...
**写/不写决策（v1.3.0）**：有明确未解问题（如 imperfect CSI、场景扩展）→写 1-2 句；无未解问题或期刊限页→省略（约 48% 论文写未来工作）。
...
1. **时态：现在完成时主导（约 15/21 篇），过去时/现在时为合法变体（约 6/21 篇）**
...
2. **不分段**：✅ 21/21 篇（100%）为单一段落；**词数 80-200 词（实测 0 篇超 200，均值约 145）**
3. **不出现新引用**：✅ 结论中不引入正文未讨论过的文献（语料 0 引用）
4. **数值支撑为可选项**：主要发现通常定性复述，数值可选（1/21 篇含数值）
5. **与 Abstract 呼应但不同**：Abstract 用现在时，Conclusion 用现在完成时为主

## 三、写作检查清单（✅ 必须 / 💡 建议）
- [✅] 开头是否属于语料高频句式（"This paper has investigated/studied..."、"In this paper, the ... has been investigated..."、"In this paper, we investigated..." 等四型之一）
- [✅] 时态是否正确（现在完成时回顾工作为主，过去时/现在时变体合法）
- [✅] 是否回顾了核心工作（做了什么）
- [✅] 是否总结了主要发现（发现了什么）
- [💡] 是否有与 Abstract 呼应的内容但不重复
- [💡] 是否有未来工作方向（1-2 句，约 48% 论文写）
- [✅] 是否没有引入新信息/新引用
- [✅] 是否没有过度夸大研究的重要性
- [✅] 语气是否冷静客观
- [✅] 长度是否在 80-200 词范围内（单段，实测 0 篇超 200）

## 四、第一轮蒸馏补充（语料发现的新模式）

### 4.1 多层数值对比（💡 低频，仅 FL 篇语料实证）
在要素 3 中可新增多层对比模板（仅当结论需要量化总结时使用）：
```
...
### 4.5 被动语态确认句式（✅ 语料实测，注意例句以语料实证为准）
```
- "It has been shown that..."（现在完成时，确认已获得的结果，语料 2-3 处）
...
- 结论需正文结果数据；用户未提供结果时以 `[待补: 主要发现]` 标注或询问，**不虚构发现/数值**（语料 21/21 结论无新引用，不可补造）；
- 数值总结为可选项（语料仅 1/21 篇含数值），用户未提供数值时不编造；
- 80 词为去极值建议下限（实测最短 60-70 词），60-79 词仅提示不硬拒。