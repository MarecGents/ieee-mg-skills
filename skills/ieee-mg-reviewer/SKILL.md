...
### 证据不足标注（v1.3.0 全 check 落地）
- 对无语料依据的规范（如 System Model 相关检查）在报告中标注"存疑/通用规范"；
- 无法客观判定的检查项标注"需人工复核"，不强行给结论；
- **⚠ 类别通用规则**：各 check 清单中的纯主观检查项（如"过渡自然""逻辑递进"）默认属 ⚠ 存疑/需人工复核，在报告条目中标注证据强度；**⚠ 不受 severity 过滤，始终单独列出**。

### 停止/降级条件
- 用户只要求检查某章节时，不审核其他章节（scope 裁剪；**全局检查仅针对该章执行**）；
- severity=light 时仅报告 🔴；severity=normal 报告 🔴+🟡；severity=strict 报告 🔴+🟡+🟢；**⚠ 三类均始终列出**；
- **统计表裁剪**：severity 过滤后统计表只统计输出级别 + ⚠ 行；
- 审核中遇到通篇性错误（如全文时态混乱）时，在总体评价中概括说明，不逐句重复列举；
- **范围外技术问题**：发现技术真实性/数据可疑等范围外问题时单独标注"⚠ 技术问题（范围外，建议人工复核）"，不纳入风格问题统计。
...
1. **默认只读**：调用本技能时，不对用户的论文原文执行任何写入操作。所有审核结果均以审核报告形式在上下文中输出。
2. **先展示后询问**：在上下文中给出完整审核报告后，主动询问「是否需要将以上修改建议应用到原文？」
3. **授权后有限写入**：仅在用户明确授权后才可执行写入。写入范围严格限定在用户授权的边界内——不可因用户同意修改一处就将所有建议全部写入。
