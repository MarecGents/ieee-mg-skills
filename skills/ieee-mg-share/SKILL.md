...
1. **默认只读**：本层为共享参考层，不对用户论文原文执行任何写入操作。所有风格基准和术语规范仅供上层技能引用。
2. **上层约束传导**：本层的只读约束通过消费方技能（writing/polishing/reviewer/response）的文件操作安全约束传导——消费方在调用本层数据后，仍须遵守各自的只读规则。
...
4. **渐进式加载**：核心文件（style-profile, terminology, quantitative-baseline）由消费方 Step 0 始终加载（manifest `load_rules.always` 保持空，由各消费技能 SKILL.md 显式声明），其他文件按需引用
